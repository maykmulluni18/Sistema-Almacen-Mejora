...
const PdfReporte_cont = () => {
    ...
    const [id_metas1, setIdMetas1] = useState('')
    const getPedidosPecosa_p = async () => {

        const res = await axios.get(URI + id,)
        setDependencias(res.data.Meta.obra)
	...
	setIdMetas1(res.data.Meta.obra)
    }
    ...
    useEffect(() => {
        const getsumarTotal = () => {
            const total = pecosaPedidoId.map((item) => parseFloat(item.cantidad * item.inventarido_inicial?.precio || item.cantidad * item.nea_bien?.p_unitario))
...
<Text style={styles.destino} fixed>
                                    <Text fixed>DESTINO                             </Text>        :<Text>   {id_metas1} </Text>
                                </Text>
                            ...
                            <View style={styles.table} >
                                ...
                                {pecosaPedidoId.map((x, i) =>
                                    ...
                                    <View style={styles.tableRow} key={`BR${i}`}>
                                        ...
                                        <View style={styles.tableCol_unidad}>
                                            <Text style={styles.tableCell}>{x?.medida}</Text>
                                        </View>
                                        <View style={styles.tableCol_fte}>
                                        ...
                                        <View style={styles.tableCol_total}>
                                            <Text style={styles.tableCell}>{Number.parseFloat(x.cantidad * x.inventarido_inicial?.precio || x.cantidad * x.nea_bien?.p_unitario).toFixed(3)}</Text>
                                        </View>
                                        <View style={styles.tableCol_codigo}>
